...
A_img_paths = natsort.natsorted(A_img_paths)
...
for i in range(181,200):
    ...
    B_img_paths = py.glob(py.join(path_per_epoch,'train_A2B'), '*.png')
    # B_img_paths.sort(key=lambda x: len(x))
    B_img_paths = natsort.natsorted(B_img_paths)
    ...
    for n, (input_image, target) in zipeed_dataset.enumerate():
        ...
        im2 = im.convert_image_dtype(target, tf.float32) 
         '''
        psnr2 = tim.psnr(input_image, target, max_val=1.0)
        for sc in psnr2:
            name = py.name(A_img_paths[idx])
            sum_sc += sc.numpy()
            idx += 1

    avg_sc = (sum_sc/len(A_img_paths))
    f = open('','a', newline='')
    wr = csv.writer(f)
    wr.writerow([i, avg_sc])
    f.close()
    '''
